    ...
    <div className="space-y-6 animate-[fadeIn_0.5s_ease-out] pb-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-5 px-1">
          ...
          <button className="text-brand-500 text-[15px] font-bold flex items-center gap-1 hover:gap-2 transition-all">
        ...
        {meals.length === 0 ? (
          ...
          <div className="space-y-4">
            {meals.slice(0, 3).map((meal) => (
              ...
              >
                <div 
                  className={`bg-white p-4 rounded-[28px] border shadow-sm flex items-center gap-4 transition-all hover:shadow-md ${
                      meal.isBabyFood ? 'border-indigo-50' : 'border-gray-100'
                  }`}
                >
                  <div className={`w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0 shadow-inner relative ${
                      meal.isBabyFood ? 'bg-indigo-50' : 'bg-gray-100'
                  }`}>
                    {meal.image ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[11px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg ${
                            meal.isBabyFood ? 'bg-indigo-100 text-indigo-600' : 'bg-brand-50 text-brand-600'
                        }`}>
