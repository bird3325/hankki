...
const Social: React.FC<SocialProps> = ({
                ...
                <>
                    <div className="min-w-[85%] flex-shrink-0 snap-center bg-gradient-to-br from-violet-500 to-fuchsia-600 p-6 rounded-[28px] shadow-lg shadow-violet-200 text-white flex items-center justify-between relative overflow-hidden transition-transform duration-300 hover:scale-[1.02]">
                        <div className="relative z-10">
                            ...
                            <p className="text-[14px] text-violet-100 opacity-90">
                    ...
                    <div className="min-w-[85%] flex-shrink-0 snap-center bg-white p-6 rounded-[28px] shadow-soft border border-gray-100 flex items-center justify-between relative overflow-hidden transition-transform duration-300 hover:scale-[1.02]">
                        <div className="relative z-10">
                            ...
                            <p className="text-[32px] font-black text-gray-800 mb-1">{cleanStreak}일 연속 🥗</p>
                            <p className="text-[14px] text-gray-400">목표 칼로리 달성 중!</p>
                        </div>
                    ...
                    <div className="min-w-[85%] flex-shrink-0 snap-center bg-white p-6 rounded-[28px] shadow-soft border border-gray-100 flex items-center justify-between relative overflow-hidden transition-transform duration-300 hover:scale-[1.02]">
                        <div className="relative z-10">
                            ...
                            <p className="text-[32px] font-black text-gray-800 mb-1">{monthlyBurned.toLocaleString()} <span className="text-[14px] font-normal text-gray-400">kcal</span></p>
                            <p className="text-[14px] text-gray-400">이번 달 누적 소모량</p>
                        </div>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                ...
                                <div className="grid grid-cols-2 gap-4 items-end">
                                    <div>
                                        ...
                                        <p className="text-[32px] font-black leading-none">{baby.uniqueIngredientsCount}<span className="text-[15px] font-medium ml-1">종</span></p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[11px] font-bold text-indigo-100 mb-1 opacity-80 uppercase tracking-tighter">주간 활동량</p>
                                        <p className="text-[18px] font-black truncate leading-tight">{baby.countThisWeek} 끼니</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    const renderFeedList = (items: Meal[], tabType: 'family' | 'baby') => (
        <div className="space-y-4">
            <h3 className="text-[18px] font-bold text-gray-800 px-1 flex items-center justify-between">
            ...
            <div className="space-y-5">
                {items.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-[32px] border-2 border-dashed border-gray-100">
                        ...
                        <p className="text-gray-400 text-[14px] font-medium mb-1">
                            ...
                            <div key={meal.id} className={`rounded-[32px] overflow-hidden shadow-soft transition-all duration-300 border ${isBabyFood ? 'bg-white border-indigo-100' : 'bg-white border-gray-100'}`}>
                                <div className="p-5 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-full overflow-hidden border-2 flex items-center justify-center shadow-sm ${isBabyFood ? 'border-indigo-100 bg-indigo-50' : (isMe ? 'border-brand-100' : 'border-gray-100')
                                            }`}>
                                            {isBabyFood ? (
                                        ...
                                        <div>
                                            <div className="flex items-center gap-1.5">
                                                <p className={`text-[14px] font-extrabold leading-none ${isBabyFood ? 'text-indigo-900' : 'text-gray-900'}`}>
                                                    {isBabyFood ? (
                                                        <span>
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="mb-2">
                                        <p className="text-[13px] font-bold text-gray-800">좋아요 {meal.likes.length}개</p>
                                    </div>
                                    <div className="mb-5">
                                        {isBabyFood && meal.babyReaction && (
                                            <div className="flex items-center gap-2 mb-2">
                                                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[12px] font-bold ${meal.babyReaction === 'good' ? 'bg-green-50 text-green-700 border-green-100' :
                                        ...
                                        <p className="text-[14px] text-gray-600 font-light">{meal.description}</p>
                                        {meal.ingredients && meal.ingredients.length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 mt-3">
                                                {meal.ingredients.map((ing, i) => (
                                                    <span key={i} className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${isBabyFood ? 'text-indigo-600 bg-indigo-50 border border-indigo-100' : 'text-brand-600 bg-brand-50 border border-brand-100'}`}>#{ing}</span>
                                                ))}
                                            </div>
                                        )}
                                    </div>
            ...
            {settings.enableBabyMode && (
                <div className="flex bg-gray-100/80 p-1.5 rounded-2xl shrink-0">
                    <button
                        ...
                        className={`flex-1 py-3 text-[15px] font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${activeFeedTab === 'family'
                    ...
                    <button
                        ...
                        className={`flex-1 py-3 text-[15px] font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 ${activeFeedTab === 'baby'
